...
if [ ! -z $2 ] ; then DB_NAME_ORIG=$2; elif [ ! -z $1 ] ; then DB_NAME_ORIG=$1 ; else read -p "Enter mysql data base name: " DB_NAME_ORIG; fi
DB_NAME=$(echo $DB_NAME_ORIG | sed "s/-/_/g")
read -p "Enter website domain: " DOMAIN_NAME;

mkdir -p ~/export
if $(mysqldump -hlocalhost -uroot -padmin16idc $DB_NAME_ORIG > ~/export/$DB_NAME.sql); then
  ...
  echo "la base de donnée $DB_NAME_ORIG n'a pu être sauvegardée" && exit 1
fi
